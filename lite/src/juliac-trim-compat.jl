# `juliac --trim=safe` compatibility overrides shared by the farm's Lambda entry
# scripts (broker, bot). Compiled into the binaries only — the packages and their
# tests never load this file. Include it at the very top of a juliac main.jl,
# *before* loading the application module: module `__init__`s and precompiled
# callers must already resolve to the replacement methods (this file loads
# Downloads/JSON itself, after the Base overrides are in place).
#
# Overridden here:
#  - Base.Libc.Libdl.dlopen(::LazyLibrary): stock version has untyped fields and
#    is reached dynamically by the ccall machinery (registered as an entrypoint)
#  - Base.errormonitor / Base.Timer(cb, ...): error printing without the dynamic
#    display_error/showerror machinery
#  - Base.sync_end: Task-only waiting (the `wait(::Any)` branch is unverifiable);
#    registered as an entrypoint (precompiled callers reach it dynamically)
#  - Downloads.request: verbatim copy with `@sync` manually expanded so the whole
#    request path is compiled fresh under the verifier's eyes
#  - Downloads.easy_hook / Downloads.Curl.set_seeker: drop `Union{Function,Nothing}`
#    dynamic calls the broker/bot never use
#  - JSON.Parsers.xparse2(Float64, ::String, ...): strtod-based float parsing (the
#    generic Parsers path uses Base.inferencebarrier and can never verify)
